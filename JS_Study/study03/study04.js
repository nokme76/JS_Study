...
function func1()
{
    console.log(`함수 func1 에서 사용한 gValue : ${gValue}`);
}
// 지역 변수는 함수(코드블럭)안에서 선언하고 사용하는 것은 문제가 없음
function func2()
{
    let lValue = 10;
    console.log(`함수 func2 에서 사용한 lValue : ${lValue}`)
}
// 함수 안에서 지역 변수 선언 시 전역 변수와 동일한 이름을 사용하면 지역 변수가 우선 순위를 가짐
function func3()
{
    let lValue = 10;
    let gValue = 20; //우선 순위 지역변수
    console.log(`함수 func3에서 사용한 lValue : ${lValue}`)
// 전역 변수와 같은 이름의 지역 변수를 선언했기 떄문에 전역 변수 보다 우선순위가 높은 지역 변수를 호출
...
// 특정 함수 안에서 전역 변수와 같은 이름의 지역 변수를 선언 했더라도 해당 함수 밖에서는 지역 변수를 사용할 수 없으므로 전역 변수가 사용됨
